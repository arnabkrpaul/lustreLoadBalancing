...
def metric_init(name, config_file, is_subscriber=False,
    ...
    if is_subscriber is False:
        ...
        stats_logger = logging.getLogger("oss_lnet_stats.%s" % __name__)
        stats_logger.propagate = False
        stats_logger_name = G.save_dir+os.sep+"oss_lnet_stats_log.txt"
...
def read_lnet_stats(f):
    ...
    logger.warn("path %s" % pfile)
    with open(pfile, "r") as f:
            for line in f:
                chopped = line.split()
                if chopped[3]:
                    ret["send_count"] = chopped[3]
                if chopped[4]:
                    ret["recv_count"] = chopped[4]
                if chopped[7]:
                    ret["send_length"] = chopped[7]
                if chopped[8]:
                    ret["recv_length"] = chopped[8]
